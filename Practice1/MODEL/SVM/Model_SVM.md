...
8. Phân tích vai trò của sơ đồ hội tụ (Loss Curve)
...
* **Số lượng Epochs (`n_iters`)**: $10$
* **CV Accuracy đạt được**: **99.45%** (Độ lệch chuẩn cực thấp $\pm0.35\%$).
...
| :--- | :--- | :--- |
| **True Positive (TP)** | Hệ thống bắt trúng chính xác email Spam | **811** |
...
| **False Negative (FN)** | Hệ thống bỏ sót (lọt lưới) email Spam | **2** |

### Các chỉ số đo lường hiệu năng
* **Accuracy (Độ chính xác tổng thể)**: **99.57%** $\rightarrow$ Mô hình phân loại chính xác hầu như tuyệt đối toàn bộ email.
...
* **Recall (Độ nhạy / Tỷ lệ bắt sạch Spam)**: **99.75%** $\rightarrow$ Chỉ lọt lưới vỏn vẹn 2 email rác trên tổng số 813 email rác thực tế.
* **F1-Score**: **99.57%** $\rightarrow$ Giá trị trung bình điều hòa giữa Precision và Recall đạt trạng thái cân bằng lý tưởng.
* **False Positive Rate (Tỷ lệ chặn nhầm)**: **0.61%** $\rightarrow$ Đạt sát mục tiêu tối thượng của các hệ thống lọc mail ($< 1\%$).

---

## 9. Phân tích vai trò của sơ đồ hội tụ (Loss Curve)
Mô hình tích hợp phương thức `plot_loss()` nhằm mục đích giám sát đồ thị của giá trị mất mát tổng thể qua từng chu kỳ huấn luyện.

### Mục đích cốt lõi của việc trực quan hóa sơ đồ hội tụ:
1.  **Kiểm tra tính đúng đắn của Gradient**: Giá trị Loss thực tế giảm mạnh từ **0.3270** (Epoch 1) xuống **0.0683** (Epoch 10). Điều này chứng tỏ thuật toán cập nhật gradient ngẫu nhiên (SGD) đang đi đúng hướng xuống đáy an toàn của hàm mục tiêu thay vì bị phân kỳ.
2.  **Xác định điểm dừng huấn luyện thích hợp**: Tốc độ giảm của Loss chậm dần và có xu hướng đi ngang từ epoch thứ 8 đến epoch thứ 10. Việc dừng ở 10 epoch là quyết định tối ưu, giúp mô hình đạt độ hội tụ sâu mà không tốn chi phí tài nguyên tính toán không cần thiết.
3.  **Phát hiện Overfitting / Underfitting**: Đường cong Loss đi xuống mịn màng, kết hợp với độ chính xác trên tập validation chéo và tập test đồng nhất ở mức $>99\%$ khẳng định mô hình đã học được tổng quát hóa đặc trưng từ các từ khóa TF-IDF, hoàn toàn không bị học vẹt (Overfitting).