...
* **Tập huấn luyện (Train Set - Cân bằng):** Bao gồm tổng cộng **6,552 mẫu**. Trong đó có **3,249 mẫu Spam** và **3,303 mẫu Ham**. Tỷ lệ lệch xấp xỉ $1:1$.
...
| :--- | :---: | :---: | :---: |
| **Accuracy (Độ chính xác tổng thể)** | 98.66% | **99.63%** | 99.21% |
| **Precision (Độ chuẩn xác bắt Spam)** | 99.38% | **99.39%** | 99.38% |
| **Recall (Tỷ lệ bắt sạch Spam)** | 97.91% | **99.88%** | 99.02% |
| **F1-Score (Độ cân bằng mô hình)** | 98.64% | **99.63%** | 99.20% |
...
1.  **Phân tích sâu cấu trúc ma trận nhầm lẫn (`plot_confusion_matrix`):**
    ...
    * Sự vượt trội về mặt hình học của SVM thể hiện ở ô bắt trúng thư rác ($TP = 812$ mẫu), chỉ để lọt lưới duy nhất đúng $FN = 1$ mẫu thư rác trên tổng số 813 mẫu kiểm thử, minh chứng cho sức mạnh tối ưu hóa lề mềm SGD từ gốc.
2.  **Định hình cán cân đánh giá qua biểu đồ cột so sánh chỉ số:**
    * Biểu đồ cột cụm cho thấy sự đồng đều và tiệm cận mức hoàn hảo ($>98\%$) của cả ba mô hình trên một tập dữ liệu cân bằng. Đồ thị trực quan giúp khẳng định cấu trúc SVM từ gốc đứng đầu toàn diện trên mọi khía cạnh đo lường chỉ số với điểm số F1-Score đạt mức kỷ lục 99.63% và độ chính xác tổng thể 99.57%, xếp ngay sau là Logistic Regression (99.20%) và cuối cùng là Naive Bayes (98.64%).