...
* **Mô hình SVM:** Được cấu hình tối ưu với `learning_rate=0.05`, hệ số regularization cực nhỏ `lambda_param=0.0001` để nới rộng lề phân cách, huấn luyện tinh gọn trong `n_iters=70` epoch để tránh overfit dữ liệu lệch.
* **Mô hình Logistic Regression:** Sử dụng ngưỡng phân loại tùy biến thực nghiệm được tinh chỉnh lên mức `threshold=0.47` (thay vì ngưỡng 0.5 mặc định) nhằm siết chặt độ chuẩn xác Precision, giảm tối đa tỷ lệ FPR để bảo vệ các email thông thường không bị chặn nhầm.
...
| :--- | :---: | :---: | :---: |
| **Accuracy (Độ chính xác tổng thể)** | 56.07% | 95.97% | **96.10%** |
| **Precision (Độ chuẩn xác bắt Spam)** | 83.94% | **99.87%** | 99.60% |
| **Recall (Tỷ lệ bắt sạch Spam)** | 14.15% | 92.00% | **92.50%** |
| **F1-Score (Độ cân bằng mô hình)** | 24.21% | 95.77% | **95.92%** |
| **False Positive Rate (Tỷ lệ chặn nhầm) ↓** | 2.66% | **0.12%** | 0.36% |
...
1.  **Phân tích sâu cấu trúc ma trận nhầm lẫn (`plot_confusion_matrix`):**
    * Giúp người phát triển nhận diện rõ số lượng mẫu cụ thể bị phân loại sai. Ví dụ, nhìn vào kết quả trực quan của **Naive Bayes**, lý do chỉ số Recall của nó cực thấp (**14.15%**) được phơi bày rõ ràng do số lượng mẫu lọt lưới thư rác quá lớn ($FN = 698$ mẫu). Mô hình Naive Bayes bị xu hướng thiên lệch nặng nề về việc dự đoán lớp đa số (Ham) đã học ở tập Train lệch.
    * Đối với **SVM** và **Logistic Regression**, biểu đồ trực quan hóa vùng mật độ màu đậm đặc tại đường chéo chính ($TN$ và $TP$) chứng tỏ cấu trúc phân tách tuyến tính bằng không gian vector TF-IDF hoạt động cực kỳ hiệu quả đối với bài toán này.
2.  **Định hình cán cân đánh giá qua biểu đồ cột so sánh chỉ số:**
    * Cho phép người thiết kế hệ thống đánh giá nhanh tương quan giữa hai chỉ số xung đột là **Precision** và **Recall**. Sơ đồ cột thể hiện cả SVM và Logistic Regression đều đạt mức Precision đỉnh cao ($>99.5\%$). Điều này đồng nghĩa với việc chỉ số **FPR** (Tỷ lệ báo động giả) được đè xuống mức cực thấp (**0.12%** ở SVM và **0.36%** ở Logistic Regression). Trong bài toán lọc thư rác, điều này mang ý nghĩa sống còn vì người dùng thà chịu đựng một vài email rác lọt vào hộp thư đến (`Recall` chưa đạt $100\%$) còn hơn là bị bộ lọc chặn nhầm một email đối tác quan trọng (`Precision` thấp, `FPR` cao).
    * Trực quan hóa đồ thị giúp khẳng định **Logistic Regression** là mô hình tối ưu nhất về mặt tổng thể khi có cột điểm F1-Score (**95.92%**) và cột điểm Accuracy (**96.10%**) vượt trội hơn hẳn hai đối thủ còn lại.