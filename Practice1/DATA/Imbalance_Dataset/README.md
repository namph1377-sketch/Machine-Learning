...
1. **Chuyển về chữ thường:** `text.lower()` - chuẩn hóa tất cả thành chữ thường.
2. **Loại bỏ tiền tố:** Xoá "RE:", "FW:", "FWD:" - các tiền tố email trả lời/chuyển tiếp.
3. **Loại bỏ "Subject:":** Xoá các dòng chủ đề email.
4. **Loại bỏ URL:** `https?://\S+|www\.\S+` - xoá các liên kết web.
5. **Loại bỏ HTML tags:** `<.*?>` - xoá các thẻ HTML.
6. **Loại bỏ ký tự không phải chữ cái:** Giữ chỉ chữ cái a-z và khoảng trắng.
7. **Dọn khoảng trắng thừa:** Loại bỏ khoảng trắng dư thừa và strip đầu/cuối.
8. **Loại bỏ Stopword:** Xoá các từ phổ biến tiếng Anh (the, a, an, etc.).
9. **Lemmatization:** Chuyển động từ về dạng gốc (running → run).
10. **Giữ từ dài >= 4 ký tự:** Loại bỏ từ quá ngắn để tập trung vào từ có nghĩa.
...
* Quy mô từ vựng lớn sẽ dẫn đến số chiều dữ liệu cao sau vectorization, cần giới hạn bằng tham số `max_features`
...
* **Tham số chính:**
  ...
  * `min_df=2` - Chỉ giữ từ xuất hiện ít nhất 2 emails (loại bỏ từ quá hiếm)
  * `ngram_range=(1,2)` - Dùng unigram (1 từ) và bigram (2 từ liên tiếp)
  * `token_pattern=r'\b\w{4,}\b'` - Chỉ giữ từ có ít nhất 4 ký tự
...
* **Kết quả sau vectorization:**
  * Train shape: `(4556, n_features)` - 4556 email, mỗi email được biểu diễn bằng n_features chiều
  * Test shape: `(1139, n_features)` - tương ứng với test data
  * Vocabulary size: số lượng từ được giữ lại sau các bộ lọc
